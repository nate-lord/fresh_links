		...
		funcs: {
			...
			addCustomScrollBar: function() {
				...
				*/
				...
				/* see updateOnSelectorChange
					 http://manos.malihu.gr/jquery-custom-content-scroller/
					 for when new content is added */
				/*
					...
					if ( l ) {
						...
						$.ajax({
							...
							success: function( data ) {
								...
								/*
								!*
								if ( catData.frozenCat ) {
									S[ 'frozen' ][ 'cats' ][ 'cat' + catData.frozenCat.id ] = catData.freshCat;
								}
								*/
							},
					...
					*/
					...
					$El.main.on( C.click, '.expandableButton, .postHeaderText', function( e ) {
						...
						*/
						...
						if ( !isOnInfoPage && !isPostOpen && $expandableContentWrap.hasClass( 'empty' ) ) {
							...
							/*
							if ( $post.hasClass( 'hasContent' ) ) {
								F.getPostFromDb( $post, id );
							} else {
								F.getPostFromWeb( $post );
							}
							*/
					...
					$El.main.on( C.click, '.closePost', function( e ) {
						...
						*/
						...
						F.togglePostButton( $expandableContentWrap.prev(), true, animationDuration );
					});
					...
					$( '#deletePostsPopUp [type="checkbox"] + .style' ).on( C.click, function() {
						/**
						* description
						*
						* @calls : 
					...
					$( '#deletePostPopUp [type="checkbox"] + .style' ).on( C.click, function() {
						/**
						* description
						*
						* @calls : 
					...
					$El.contentWrap.on( C.click, '#createFeed', function() {
						...
						*/
						...
						// !*	feedOb.doesMakeFrozenCat = true;
							/* !*
							if ( urlTestResults.isFrozenUrlInUse ) {
								feedOb.doesMakeFrozenCat = false;
								// !* feedOb.additionalAction = 'link_fresh_cat_with_preexisting_frozen';
							} else {
								feedOb.doesMakeFrozenCat = true;
							}
							*/
			...
			cacheEls: function() {
				...
				*/
				...
				if ( S.isUserLoggedIn ) {
					...
					$El.daysFreshWrap = $( '#daysFreshWrap' ); // ? i don't think this exists
					$El.dropUser = $( '#dropUser' );
					$El.feed = $( '#feed' );
					$El.homeNavWrap = $( '#homeNavWrap' );
					$El.searchWebForFeeds = $( '#searchWebForFeeds' );
					$El.settingsForm = $( '#settingsForm' );
					$El.toggleSettings = $( '#toggleSettings' ); // ? i don't think this exists
					$El.titleTag = $( 'title' );
				} else {
					$El.login = $( '#login' );
			...
			cleanPost: function( $post, $wrap, url ) {
				...
				*/
				...
				//F.insertPostInDataBase($result.prop('outerHTML'));
			...
			convertPreTags: function() {
				...
				*/
				...
				<pre> isn't allowed in <p> (it causes a render error), but a <figure> styled like a <pre> is.
			...
			cURLTest: function() {
				...
				*/
				...
				$.ajax({
					...
					data: {
						action: 'cURL_test',
						rssUrl: 'http://feeds.gawker.com/io9/vip'
					},
					success: function( data ){
						// ?
						//var data = JSON && JSON.parse($.trim(data)) || $.parseJSON($.trim(data));
						// console.log(data);
						
						//F.getPostMetaFromWeb(ob);
					}
				});
			},
			*/
				...
				*/
				...
				$.ajax({
					...
					success: function( data ) {
						...
						/*
						!*
						if ( data.didDeleteFrozen ) {
							delete S[ 'frozen' ][ 'cats' ][ 'cat' + data.frozenCatId ];
						}
						*/
					}
				});
			...
			errorCheckName: function( name ) {
				...
				*/
				...
				/*
				!*
				frozenCats = Object.keys( S.frozen.cats );
				
				if ( frozenCats.length) {
					for ( i = 0, l = frozenCats.length; i < l; i++ ) {
						inUseName = S[ 'frozen' ][ 'cats' ][ frozenCats[ i ] ][ 'display_name' ]
							.toLowerCase();
							
						if ( name === inUseName ) {
							return 'you are currently subscribed to this frozen url';
							break;
						}
					}	
				}
			...
			errorCheckUrl: function( url ) {
				...
				*/
				...
				// ? what if you delete a fresh cat, have a frozen cat, then try to add the fresh cat again?
				// ? what if you do it under a different name?
				// answer: the frozen category changes its name automatically w. out notification
				...
				if ( frozenCats.length ) {
					for ( i = 0, l = frozenCats.length; i < l; i++ ) {
						inUseUrl = S[ 'frozen' ][ 'cats' ][ frozenCats[ i ] ][ 'rss_url' ];
						
						if ( ( URI( inUseUrl ).equals( url ) ) ||
								 ( URI( F.stripWww( inUseUrl ) ).equals (strippedUrl ) ) ) {
							result.isFrozenUrlInUse = true;
							break;
						}
					}
				}
			...
			getNewPostInfoFromRawEntries: function( entryArr, catId ) {
				...
				*/
				...
				for ( i; i < l; i = i + 1 ) {
					...
					//editedEntries[i].excerpt = entryArr[i].contentSnippet;
			...
			getPostFromDb: function( $wrap, id ) {
				/**
				* description
				*
				* @called : mouseenter / leave and click ev on .expandableButton, .postHeaderText
				...
				*
				* @params {jquery object} - $wrap : the article.postWrap
				* @params {int} - id : the data-attr id from the article.postWrap, the WP post's id
				*/
				...
				$.ajax({
					...
					url: C.ajaxUrl,
					data: {
						action: 'get_post_from_db',
						id: id
					},
					success: function( post ) {
						F.preInsertPostInPage( post, $wrap, false );
					}
				});
				*/
			},
			
			getThumbnail: function( $thePost ) {
				/**
				* description
				*
				* @called : 
				...
				*/
				...
				for( i; i < l; i = i + 1 ){
					$img = $imgs.eq(i);
					height = parseInt( $img.css('max-height'), 10 );
					width = parseInt( $img.css('max-width'), 10 );
					if( i === 0 ){
						src = $img.attr('src');
						ratio = Math.min(height, width) / Math.max(height, width);
					} else {
						newRatio = Math.min(height, width) / Math.max(height, width);
						if( newRatio > ratio ) {
							// the highest is closest to 1, most square
							ratio = newRatio;
							src = $img.attr('src');
						}
					}
			...
			insertPostInDataBase: function($post, id) {
				/**
				* description
				*
				* @called : 
			...
			preInsertPostInPage: function($post, $wrap, isInitInsertion) {
				...
				*/
				...
				here's the problem:
				you won't know when you have access to the dimensions of imgs or iframes
				you don't want to change them after the post loads up bc the height will
				change
				
				the solution:
				set an interval timer.
				add the make an id incorporating the id of the post in its name.
				do not allow the function this comment is in to continue until the timer
				is canceled.
				give all of the elements the class 'noDimensions'.
				in an interval of 50ms, check to see if the dimensions of each with
				the 'noDimensions' class have been loaded. if so, add the dimensions and remove the class.
				once there are none with the class noDimensions, continue this function.
				
				finally, once the page has been added visually turn it's html into a string
				and upload the page to the database.
				
				
				also, unrelated to above:
				any anchors with href's starting w. '#' make into spans.
			...
			showInsertedPost: function( $post, $wrap, $expandableContent ) {
				...
				*/
				...
				//$expandableContent.css( 'height', $theLoader.height() );
			//	alert($theLoader.height());
				$theLoader.remove();
				...
				$.ajax({
					...
					success: function( templatePart ) {
						...
						if ( sectionName === 'logged-in' ) {
							...
							// i think this fixes a glitch where the <title> text flashes a bunch unless you wait?
							window
								.setTimeout( function() {
									$El.titleTag
			...
			reorderDropDownMenuItems: function( menuOb, $selection ) {
				/**
				* description
				*
				* @called : 
				...
				* @page : 
				*
				* @params {object} - menuOb :
				* @params {jquery object} - $selection :
				*/
				
				/**
				* called: 
				* calls: toggleDropDownHeight(menuOb)
				* called with: 
				*
				* arg: menuOb is the menu object set in the global settings
				*			 $selection is the list item that was selected from the menu
				*
				* var: $after is
				*			 $menu is
				*			 $clearSelection is
				*			 clearSelectionIndex is
				*			 $currentSelectionWrap is
				*			 text is
				*			 selectionIndex is
				*			 $selections is
				*			 startIndex is
				*
				* reorderDropDownMenuItems
				*/
				...
				var $after,
						$menu = $( '#' + menuOb.id ),
						$clearSelection = $menu.find( $( '.clearSelection' ) ),
						clearSelectionIndex = $clearSelection.index(),
						$currentSelectionWrap = $menu.find( $( '.currentSelectionWrap' ) ),
						text,
						selectionIndex = $selection.index(),
						$selections = $menu.find( $( '.selections' ) ),
						startIndex;

				// note that these are order-dependent for detach/insert
				
				if ( !clearSelectionIndex && selectionIndex ) {
					// if clearSelection's index is equal to 0 and selection's index is not
					
					text = $currentSelectionWrap.data( 'dynamic_text' );
					
					$currentSelectionWrap
						.detach()
						.addClass( 'selection' )
						.removeClass( 'currentSelectionWrap' )
						.data( 'dynamic_text', $currentSelectionWrap.text() )
						.empty()
						.text( text );
													 
					$selection
						.detach()
						.addClass( 'currentSelectionWrap' )
						.removeClass( 'selection' )
						.append( '<div class="openCloseArrow"></div>' )
						.wrapInner( '<div class="currentSelection"></div>' )
						.prependTo( $selections );
										
					$currentSelectionWrap.insertAfter( $selection );
					
					menuOb.currentSelectionWrap = $selection;
				} else if ( clearSelectionIndex === 1 &&
								 		selectionIndex > 1 )
				{
					text = $currentSelectionWrap.text();
					startIndex = $currentSelectionWrap.data( 'start_index' );
					...
					$after = $selections.find( $( '.selection[data-start_index="' + ( startIndex - 1 ) + '"]' ) );

					if ( !$after.length ) {
						$after = $selections.find( $( '.selection[data-start_index="' + ( startIndex - 2 ) + '"]' ) );
					}
					
					$currentSelectionWrap.insertAfter( $after );
								 		...
								 		selectionIndex === 1 )
				{
					...
					text = $clearSelection.data( 'dynamic_text' );
					
					$clearSelection
						.detach()
						.addClass( 'currentSelectionWrap' )
						.removeClass( 'selection' )
						.data( 'dynamic_text', $clearSelection.text() )
						.text( text )
						.append( '<div class="openCloseArrow"></div>' )
						.wrapInner( '<div class="currentSelection"></div>' )
						.prependTo( $selections );
					
					if ( startIndex === $selections.children().length ) {
						// if it was the last
						$currentSelectionWrap.appendTo( $selections );
					} else {
						$currentSelectionWrap
							.insertBefore( $selections.children().eq( startIndex ) );
					}
					
					menuOb.currentSelectionWrap = $clearSelection;
				}			 
				F.toggleDropDownHeight( menuOb );
			...
			repeatingCloudAnimation: function() {
				...
				*/
				...
				// WAS ORG HERE S.cloudAnimationCounter = S.cloudAnimationCounter + 1;
			...
			revertUnsavedFeedInfo: function( ob ) {
				...
				*/
				...
				// NOTE: was quickly editted to remove url edit changes too.
				// this could be cleaned up as a result.
				
				
				// ie9 doesn't support rgba.
				// clearColor & colorAlpha account for that.
				// see velocity docs for color animation info.
			...
			setConfig: function() {
			  ...
			  C.cloudsWidth = window.screen.width;
				...
				/*
				the master category object is obtained from the Wp object in the php generated <script> tag
				
				the master category object: {
					id: the category's WP id,
					name: the category's name - the username + '_master'
				}
				*/
				C.master = Wp.master;
			...
			setDropDownSettings: function( menuOb, $menu ) {
				/**
				* description
				*
				* @called : 
				...
				* called with: 
				*
				* arg: menuOb is the menu object in the global settings object to populate
				*			 $menu is the cached drop down elem to set
				*
				* setDropDownSettings
				*/
				menuOb.currentSelectionWrap = $menu.find( $( '.currentSelectionWrap' ) );
				menuOb.id = $menu.attr( 'id' );
				menuOb.menu = $menu;
				menuOb.shadow = $menu.find( $( '.style' ) );
				menuOb.height = $menu.find( $( '.selections' ) ).outerHeight();
				
			},
			
			setFeaturedImageAndThumbnail: function( $imgs, $wrap ) {
				/**
				* description
				*
				* @called : 
				...
				*/
				...
				if ( S.isUserLoggedIn ) {
					...
					*/
					...
					// ? S.freshCategoryIds = Wp.freshCategoryIds;
			...
			setUpScrollEverywhere: function( isInt ) {
				/*
				var margin,
						delay = 0;
				...
				margin = ( S.isScreenLarge ? 20 : 0 );
				
				if ( isInt ) {
					delay = 425;
					
					$El.timeline = $( '#mCSB_1_container' );
				
					$El.scrollGrip = $( '#mCSB_1_dragger_vertical' );
					
					console.log('called');
				}
				
				S.scrollGripScrollMax = ( S.isScreenLarge ? S.vpHeight - 100 : S.vpHeight - 84 );
				
				// hacky but prevents a gap in the bottom on the timeline during hard refresh / first load
				window.setTimeout(
					function() {
						S.timelineHeight = $( '#currentMainSectionWrap' ).height();
						S.timelineScrollMax = S.vpHeight - S.timelineHeight - margin;
				}, delay);
				*/
			},
			...
			toggleDropDownHeight: function( menuOb ) {
				/**
				* description
				*
				* @called : 
				...
				*/
				...
				/**
				* called: ev click on .currentSelection,
				*				 	reorderDropDownMenuItems(menuOb, $selection)
				*
				* arg: menuOb is the menu object to reset the members of
				* var: height is the new height of the drop down
				*
				...
				*/
				...
				if ( menuOb.isOpen ) {
					// closes it
					// height is the height of the current selection
					height = menuOb.currentSelectionWrap.outerHeight();
					menuOb.isOpen = false;
					...
					// opens it
					// height is equal to the height of the menu
					height = ( menuOb.canScroll ? 'auto' : menuOb.height );
					menuOb.isOpen = true;
					...
					menuOb.shadow
						.css( { top: menuOb.currentSelectionWrap.outerHeight() } );
				}
			...
			toggleFeedEditablity: function( $feedInfo ) {
				/**
				* description
				*
				* @called : 
				...
				*/
				...
				var hasNonSavedFeedInfoChanged,
						$nonSavedFeedInfo = $feedInfo.siblings( '.canSave' );
						
				// ? isn't called anywhere
			},
			
			toggleLoadingFeedback: function( control ) {
				/**
				* description
				*
				* @called : 
				...
				*/
				...
				/* starts or stops loading animation */
			...
			togglePostButton: function( $header, wasClicked, delay ) {
				/**
				* description
				*
				* @called : mouseenter / leave || click ev on .expandableButton, .postHeaderText,
				*						click ev on .closePost
				*
				* @group : toggle post / interactive style
				* @page : timeline
				*
				* @params {jquery object} - $header : the header.expandablePostHeader (the btns in the timeline)
				* @params {bool} - wasClicked : was the event a click ev? otherwise it's a mouseover (problematic bc of mobile)
				* @params {int} - delay : the a delay in the animation (optional)
				*/
			...
			trySignInOrRegisterUser: function( args ) {
				...
				*/
				...
				if ( args.userName.length === 0 ) {
					F.showMalformedInputWarning( $usernameError, $username, 'field is empty' );
				} else if ( args.userName.length > noSpacesUsername.length ) {
					F.showMalformedInputWarning( $usernameError, $username, 'username can\'t have spaces' );
				}
				
				if ( args.password.length === 0 ) {
					F.showMalformedInputWarning( $passwordError, $password, 'field is empty' );
				} else if ( args.password.length <= 4 ) {
					if ( args.action === 'register_user' ) {
						$input = args.wrapper.find( $( 'input' ) ).eq( 1 );
						$error = $input.next();
						
						F.showMalformedInputWarning( $passwordError, $password, 'pass must be more than 4 chars' );
					} else if ( args.action === 'login_user' ) {
						$input = args.wrapper.find( $( 'input' ) ).eq( 1 );
						$error = $input.next();
				...
				*/
				...
				$.ajax({
					...
					success: function( data ) {
						...
						// !* S.doesWarnFrozenCategoryDrop = data.settings.doesWarnFrozenCategoryDrop;
			...
			tryToEditExisitingCatName: function( $feedInfo ) {
				...
				*/
				...
				the error needs to prevent the transform to not editable
			...
			turnOnScrollEverywhere: function( isInt ) {
				/*
				F.setUpScrollEverywhere( isInt );
				...
				$( '#main' ).on( 'mousewheel DOMMouseScroll', function( e ) {
					var wheelDelta,
							direction = ( C.isMacAndNonMobile ? -1 : 1 ),
							scrollGripTop,
							scrollGripCurTop = $El.scrollGrip.css( 'top' ),
							timelineTop,
							timelineCurTop = parseInt( $El.timeline.css( 'top' ), 10 );
					
					// might be undefined	
					scrollGripCurTop = ( scrollGripCurTop === undefined ? 0 : parseInt( scrollGripCurTop, 10 ) );
					
					wheelDelta = ( e.originalEvent.wheelDelta !== undefined ? e.originalEvent.wheelDelta : e.originalEvent.detail );
					wheelDelta = wheelDelta * 3; // changes the speed
					wheelDelta = wheelDelta * direction; // fixes so scrolls correctly on non mobible macs
					
					timelineTop = timelineCurTop + wheelDelta;
					timelineTop = ( timelineTop > 0 ? 0 : timelineTop );
					timelineTop = ( timelineTop < S.timelineScrollMax ? S.timelineScrollMax : timelineTop );
					
					$El.timeline.css( 'top', timelineTop );
					
					scrollGripTop = Math.round( timelineTop / S.timelineScrollMax * S.scrollGripScrollMax );
					scrollGripTop = ( scrollGripTop > S.scrollGripScrollMax ? S.scrollGripScrollMax : scrollGripTop );
					
					$El.scrollGrip.css( 'top', scrollGripTop );
				...
				*/
				...
				$.ajax({
					...
					success: function( data ) {
						...
						!*
						if ( catIds.frozenId ) {
							S[ 'frozen' ][ 'cats' ][ 'cat' + catIds.frozenId ][ 'display_name' ] = newBaseName;
						}
						*/
					}
				});
